...
class FinancialDataCollector:
    ...
    def collect(self, tickers: List[str], start_date: Optional[str] = None, end_date: Optional[str] = None) -> pd.DataFrame:
        ...
        # Download ONE ticker at a time to avoid MultiIndex columns
        for ticker in tickers:
            try:
                logger.info(f'  Downloading {ticker}...')
                # IMPORTANT: Download single ticker to get simple columns
...
with open('src/data/collectors.py', 'w', encoding='utf-8') as f:
    f.write(content)

print(' collectors.py recréé')
